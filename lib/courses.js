...
class Courses {
    ...
    async deadlinesReload() {
        ...
        for (const deadlineJSON of deadlinesJSON) {
            // reject the deadline if it doesn't have a course
            if (!this.courses[deadlineJSON.course]) {
                rejected++;
                continue;
            }
        ...
        this.bot.log.log(
            "COURSES", 
            `Reloaded ${this.deadlines.length} deadlines from disk. ` +
            `${rejected} rejected.`);
    }
