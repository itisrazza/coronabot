...
class Courses {
     ...
     * @param {(): string} warning A function which returns a warning message.warning
     * @this {Deadline}
     */
    onDeadlineWarning(warning) {
    ...
    scheduleJobsEnable() {
        for (let i = 0; i < Deadline.warnings.length; i++) {
            ...
            this.scheduleJobs.push(job = schedule.scheduleJob(
                this.scheduleName() + ":" + i,
                warningTime,
                Deadline.deadlineCallback.bind(this, message)
            ));
        }
    }
...
class Lecture {
    ...
    scheduleJobsEnable() {
        ...
        this.scheduleJob = schedule.scheduleJob(
            this.scheduleName(),
            this.cron,
            Lecture.lectureCallback.bind(this)
        );
    }
