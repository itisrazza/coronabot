...
class Announcements {
    ...
    constructor(config, client, bot) {
        ...
        this.imapClient.connect()
            .then(() => {
                console.log('EMAIL: Connected to server')
                
                // select the mail box
                this.imapClient.selectMailbox('INBOX').then(mailbox => {
                    console.log('EMAIL: Got the mailbox')
    ...
    async checkMailbox() {
        console.log('EMAIL: Checking mailbox...')
        ...
        for (let i = 1; i <= boxDeets.exists; i += blockSize) {
            ...
            console.log(`EMAIL: Downloaded mailbox ${scale}`)
        ...
        console.log(`EMAIL: Done. ${newUids} new entries`)
    }
