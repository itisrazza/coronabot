...
class ClassTimes {
    ...
    constructor(config, client, bot) {
        ...
        this.warnings = {
            ...
            240: event => `${event} will be due soon`,      // 8PM of the day
            900: event => `${event} will be due tonight`,   // 9AM of the day
            1680: event => `${event} will be due tomorrow`,         // 8PM of the day
            2340: event => `${event} will be due tomorrow evening`, // 9AM day before
    ...
    async initChannels() {
        ...
        this.deadlineChannel =
            await this.client.channels.fetch(channels.deadlines);
        this.lectureTimeChannel =
            await this.client.channels.fetch(channels.lectureTimes);
    }
    ...
    async importData() {
        ...
        // load csv data
        let deadlinesRaw = await fs.readFile(
            path.join(__dirname, "..", "data", "deadlines.csv")
        );
        let lectureTimesRaw = await fs.readFile(
            path.join(__dirname, "..", "data", "lecture-times.csv")
        );
        ...
        this.deadlines = deadlinesObj.map(row => {
            ...
            let warningDates = warningTimes.map(
                time => warnTimeCalc(date, time)
            );
            ...
            warningTimes.forEach((time, i) => {
                ...
                let job = schedule.scheduleJob(
                    ...
                    ((deadline, warning) => {
                        ...
                        this.deadlineChannel.send(
                            sb + warning(`${course} / ${name}`)
                        );
                ...
                if (job == null) {
                    this.bot.log.log(
                        "DEADLINE",
                        `Failed to register job for ${deadline.scheduleName()}`
                    );
                }
            });
        ...
        this.lectureTimes = lectureTimesObj.map(row => {
            ...
            let job = schedule.scheduleJob(
                ...
                ((lectureTime) => {
                    ...
                    if (role !== null) sb += `<@&${role}> - `;
                    this.lectureTimeChannel.send(
                        sb +
                        `${course} / ${name} will soon start in ${location}`
                    );
    ...
    async addDeadline(course, name, date) {
