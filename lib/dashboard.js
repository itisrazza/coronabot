
/*!
 * CoronaBot - Dashboard
 *
 * The dashboard implements a central location of sorts to send out and then
 * update information as opposed to having to run commands.
 */

const Discord = require("discord.js");
const { Job } = require("node-schedule");
const exec = require("child_process").execSync;

const MS_PER_SECOND  = 1000;
const MS_PER_MINUTE = 60 * MS_PER_SECOND;
const MS_PER_HOUR = 60 * MS_PER_MINUTE;

class Dashboard {
    /**
     * @param {object} config The configuration object (JSON)
     * @param {Discord.Client} client 
     * @param {object} bot 
     */
    constructor(config, client, bot) {
        this.config = config;
        this.client = client;
        this.bot = bot;

        // load in values that won't change during runtime
        this.package = require("./package.json");
        this.gitHash = this.getGitHash();

        // save the time the program started existing (or at least this module)
        this.startTime = new Date();

        // keep an object with messages and functions to update said messages
        this.messageAssemblers = [
            this.assembleBotInfo.bind(this),
            this.assembleToday.bind(this),
            this.assembleDeadlines.bind(this),
            this.assembleBirthdays.bind(this)
        ];
        this.messages = [];

        // set up the channel for dashboard
        /** @type {Discord.TextChannel} */ this.channel = null;
        this.initChannel();

        this.bot.log.log("DASHBOARD", "Module constructed");
    }

    /**
     * Init Discord channels (using async and magic Promise stuff) and
     * initially create the messages.
     */
    async initChannel() {
        this.channel = await this.client.channels.fetch(
            this.config.dashboard.channel
        );
        this.bot.log.log("DASHBOARD", "Got channel: " + this.channel.id);

        // clear the messages from the channel
        await this.deleteAllMessages();

        // write out new messages
        this.messageAssemblers.forEach((async (assembler) => {
            let message = await this.channel.send("Loading: " + assembler.name);
            this.messages.push({ assembler, message });

            this.bot.log.log("DASHBOARD", "Mapped message ID: " + message.id);
        }).bind(this));

        // hook in the updater
        setInterval(
            this.updateMessages.bind(this),
            this.config.dashboard.interval
        );

        this.bot.log.log("DASHBOARD", "Set up channel and messages");
    }

    async deleteAllMessages() {
        let fetched;
        do {
            fetched = await this.channel.messages.fetch({ limit: 50 });
            this.channel.bulkDelete(fetched);
        }
        while (fetched.size >= 2);
    }

    async updateMessages() {
        this.bot.log.log("DASHBOARD", "Message update triggered");
        for (const { assembler, message } of this.messages) {
            try {
                message.edit(assembler());
            } catch (e) {
                message.edit(`⚠️ Error generating ${assembler.name}:\n\`\`\`${e}\`\`\``);
                this.bot.log.log(
                    "DASHBOARD", 
                    `Failed updating "${ assembler.name }": ${ e }`
                );
            }
        }
    }

    /*
     * function below assemble the messages
     */

    /**
     * Thanks time.
     */
    assembleDeveloper() {
        return "**Developer information**\n" +
            "When @rare is working on the bot, messages will appear and disappear constantly, very likely triggering notifications (sorry).\n\n" +
            "If you don't wanna be senselessly pinged, right click on **#dev-dashboard**, mute the channel, then change the notification settings to 'Nothing'.\n\n" +
            "(messing around with perms also breaks the production bot, for which sshing into the server, `sudo su` twice to get to www-data account, finding the server pid, `kill -9` it down, and then digging up the GitHub webhook and calling that to restart is a bit of a hassle)";
    }

    /**
     * Assemble bot metadata
     */
    assembleBotInfo() {
        return "**Bot Information**\n" +
            `> Discord User: <@${this.client.user.id}>\n` +
            `> Bot Software: CoronaBot ${this.package.version}` +
            `-${this.gitHash} (https://git.io/Jf7tN)\n` +
            `> Platform: ${process.platform}\n` +
            `> Uptime: ${this.getUptime()}\n` +
            `> Process ID: ${process.pid}\n`;
    }

    /**
     * Assemble today's lectures.
     */
    assembleToday() {
        let lectures = this.bot.courses.lectures;
        
        // and filter out lectures that will occur from now on
        let upcomingLectures = lectures
            .sort((a, b) => a.next() - b.next())
            // .filter(a => {
            //     /** @type {Date} */ let date = a.next();

            //     // check if the lecture is today
            //     let dayStart = new Date(Math.floor(Date.now()
            //         - date.getHours() * MS_PER_HOUR
            //         - date.getMinutes() * MS_PER_MINUTE
            //         - date.getSeconds() * MS_PER_SECOND
            //         - date.getMilliseconds()));
            //     if (date - dayStart > 24 * MS_PER_HOUR) return false;

            //     // check if the lecture already started
            //     return date > new Date();
            // });
        
        let sb = "**Today's Lectures**\n";
        
        // message for no lectures
        if (upcomingLectures.length == 0) {
            return sb + 
                "> 🥳 That's all folks! No lectures for the rest of the day.";
        }

        for (const lecture of upcomingLectures) {
            function dateFormat(date) {
                return `${date.getHours().toString().padStart(2, "0")}:`
                    + `${date.getMinutes().toString().padStart(2, "0")}`;
            }

            sb += `> \`[${dateFormat(lecture.next())}]\` `
                + `\`${lecture.course.name}\` ${lecture.name} → `
                + `${lecture.location}\n`;
        }

        return sb;
    }

    /**
     * Assemble the string for upcoming deadlines and events
     */
    assembleDeadlines() {
        const EVENTS_PER_COURSE = 3;
        let sb = "**Upcoming Deadlines**\n\n";

        /**
         * @param {Date} date
         * @returns {string}
         */
        function dateFormat(date) {
            return `${date.getDate()}/${date.getMonth().toString().padStart(2, "0")}/${date.getFullYear()}` +
                ` ${date.getHours()}:${date.getMinutes().toString().padStart(2, "0")}`
        }

        // aggregate the assignments due soonest
        let events = {};
        let sortedDeadlines = this.bot.courses.deadlines
            .sort((a, b) => a.date - b.date)
        for (const deadline of sortedDeadlines) {
            let course = deadline.course.name;

            // check if course exists
            if (!Object.keys(events).includes(course)) {
                events[course] = [];
            }

            // stop if it's full
            if (events[course].length >= EVENTS_PER_COURSE) continue;

            // add to event list
            events[course].push(deadline);
        }

        // convert the aggregate to a string
        for (const course of Object.keys(events)) {
            sb += `> **${course}**\n`;
            for (const deadline of events[course]) {
                let { name, date } = deadline;
                sb += `> → **${name}** - ${dateFormat(date)}\n`;
            }
            sb += "\n";
        }

        return sb;
    }

    /**
     * Assemble the string for the next upcoming birthdays
     */
    assembleBirthdays() {
        return "**Birthdays**\n" +
            "The birthdays module is temporarily disabled due to conflicts with the new Courses module.";

        // this is temporary while the bday module is rewritten

        // handle birthdays here
        const UPCOMING_BIRTHDAYS_MAX = 5;
        let sb = "**Birthdays**";

        // 

        // aggregate the birthdays of people
        let dates = {};
        this.bot.birthdays.getUpcomingBirthdays().forEach(bday => {
            let day = bday.next();
            let dayString = day.toDateString();

            // stop if it's full
            if (Object.keys(dates).length >= UPCOMING_BIRTHDAYS_MAX) {
                return;
            }

            // create an array if it doesn't exist
            if (!Object.keys(dates).includes(dayString)) {
                dates[dayString] = [];
            }

            // add birthday to the list
            dates[dayString].push(bday);
        });

        // convert all of that to a string
        Object.keys(dates).forEach(date => {
            sb += `Birthdays on ${date}\n`;
            dates[date].forEach(bday => {
                let { name } = bday;
                sb += `> → ${name}\n`;
            });
            sb += "\n";
        });

        return sb + "\n";
    }

    /*
     * helper functions
     */

    /**
     * Retrieve the Git commit hash for the running version of the bot
     */
    getGitHash() {
        try {
            return exec("git rev-parse HEAD", { encoding: "utf8", cwd: __dirname + "/.." })
                .substring(0, 6);
        } catch {
            return "custom";
        }
    }

    /**
     * Retrieve the current uptime of the bot in human-readable form
     */
    getUptime() {
        // get the difference in milliseconds
        let uptimeMilli = new Date() - this.startTime;

        // get days, hours, minutes
        let seconds = uptimeMilli / 1000;
        let minutes = seconds / 60;
        let hours = minutes / 60;
        let days = hours / 24;

        // subtract the bigger things from the smaller things
        hours -= days * 24;
        minutes -= hours * 60;
        seconds -= minutes * 60;

        // round the numbers off
        days = Math.floor(days);
        hours = Math.floor(hours);
        minutes = Math.floor(minutes);

        // assemble the string together
        let sb = "";
        if (days > 0) sb += `${days} days, `;
        if (hours > 0) sb += `${hours} hours, `;
        sb += `${minutes} minutes`;

        return sb;
    }
};

module.exports = exports = Dashboard;
